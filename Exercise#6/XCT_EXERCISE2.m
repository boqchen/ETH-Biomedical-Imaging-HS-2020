...
%=========================================================================
    ...
    for phi=projection_angles
        ...
        phantom.sino(:,idx) = CalcLineIntegrals(r,phi_,phantom,ua)/(matrix+1); %projection at phi
    ...
    for phi=projection_angles
        ...
        phi = phi*pi/180;
        rs = round(x.*cos(phi) + y.*sin(phi) + floor((matrix+1)/2)+1);
        ix = find((rs > 0) & (rs < 257));
    ...
    filterH      = CalcFilter(matrix+1);   % TASK 2.3 EDIT FUNCTION
    
    for phi=projection_angles
        
        % TASK 2.3 FILL IN HERE
        offset = floor((matrix+1)/2)+1;
        phi = phi*pi/180;
        rs = round(x.*cos(phi) + y.*sin(phi) + offset);
        ix = find((rs > 0) & (rs < 257));
        filter = filterH;
        filteredprojection = ifft(ifftshift(fftshift(fft(phantom.sino(:,idx))).*filter'));
    ...
    [pp,qq]     = meshgrid(-fix(matrix/2):+fix(matrix/2));
    phantom.fft = zeros(matrix+1,matrix+1);
    
    for phi=projection_angles
    
        p(:,idx) = r*cos(phi/180*pi);
        q(:,idx) = r*sin(phi/180*pi);
        
        ft(:,idx) = R2U(phantom.sino(:,idx));
        
        idx = idx+1;
    end
   
    % Grid data to Cartesian matrix
    phantom.fft = griddata(p,q,ft,pp,qq);
...
function [filter] = CalcFilter(matrix)
    ...
    % --------------------------------------------------------------------
    filter = abs(-fix(matrix/2):+fix(matrix/2)).*hamming(matrix+1);
