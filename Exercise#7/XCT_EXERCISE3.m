...
function [] = XCT_EXERCISE3()
    ...
    Ne              = Ua*Ia*yield/e;        % #electrons          [1/s]   
    N0              = Ne*dt                % #photons            []  
    ...
    for phi=projection_angles
        ...
        phantom.ellipse(8,4) = 20 + sin(phi);
        ...
        title('Sinogram'); xlabel('phi'); ylabel('r'); 
...
function phantom = ApplyGaussianFilter(phantom,factor)
    ...
    [u,v]       = meshgrid(-fix(matrix/2):+fix(matrix/2)); 
    ...
    FWHM = matrix;
    sigma = FWHM/(2.3548*factor);
    
    % --------------------------------------------------------------------
    % Calculate Gaussian filter
    % --------------------------------------------------------------------
    % TASK 3.2 FILL IN HERE
    filter = exp(-(u.^2+v.^2)./(2*sigma^2));
