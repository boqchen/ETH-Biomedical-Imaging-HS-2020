...
function [] = SAMPLING_PATTERN_AND_CONTRAST()
    ...
    contrast_choice = input('Pls input contrast_choice = ')
    ...
    elseif contrast_choice == 1
        ...
        TR = input('Pls input TR = ');
        angle = 50 * pi/180;
        for t = 1:n_tissues
            signal(:,:,:) = signal(:,:,:) + sin(angle)*((1-exp(-TR/T1(t)))/(1-cos(angle))*exp(-TR/T1(t)))*rho(t)*tissue_map(:,:,:,t);
        end
    end
    ...
    %Sampling Extent
    sampling_extent_choice = input('Pls input sampling_extent_choice = ');
    ...
    %Sampling Density
    sampling_density_choice = input('Pls input sampling_density_choice = ');
    if sampling_density_choice == 0
        %Full Sampling
    elseif sampling_density_choice == 1
        %keep only every second horizontal line 
        for kx = 1:2:nx
            sampling_pattern(kx,:) = 0;
        end
        
    elseif sampling_density_choice == 2
        %keep only every third horizontal line 
        for kx = 1:3:nx
            sampling_pattern(kx,:) = 0;
        end
        for kx = 3:3:nx
    ...
    elseif sampling_density_choice == 3
        %keep only every second vertical line 
        for ky = 1:2:ny
            sampling_pattern(:,ky) = 0;
        end 
    
    elseif sampling_density_choice == 4
        %keep only every third vertical line 
        for ky = 1:3:ny
            sampling_pattern(:,ky) = 0;
        end 
        for ky = 3:3:ny
    ...
    elseif sampling_density_choice == 5
        %2D undersampling every second vertical and horizontal line
        for kx = 1:2:nx
            sampling_pattern(kx,:) = 0;
        end
        for ky = 1:2:ny
